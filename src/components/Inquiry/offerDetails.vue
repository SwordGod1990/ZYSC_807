    ...
    <!-- <v-title></v-title> -->
        	   ...
        	   <div class="drugInformation" >
                    ...
                    <li class="reveals">
                         ...
                         <p>报价总额：<span class="plan"></span><span class="totalNum"></span><button @click="modify($event,i.offerOrderInfoId,index)">修改</button></p>
        	   	   	  ...
        	   	   	  <li class="indication">
        	   	   	  	 <button class="keep" @click='keep($event,i.offerOrderInfoId,i.medicinalId,index)' :data-offerOrderInfoId='i.offerOrderInfoId' :data-medicinalId='i.medicinalId'>保存</button>
        	   	   	  </li>
        	   	   </ol>
        	   </div>
      	   </div>
      ...
      <div v-if='reveal' class="reveal_mark">
           <div class="reveal_main">
                ...
                <button class="Submission" @click='reveal=!reveal,submission(offerOrderId,$event)' data-offerOrderId='offerOrderId' :disabled="isDisable">提交</button>
           </div>          
      </div>
      <!-- 弹框 -->
...
export default {
  ...
  mounted(){
    this.sysUserId=localStorage.getItem("sysUserId")
     this.offerOrderId=this.$route.query.offerOrderId//报价单id
     console.log(this.offerOrderId)
     console.log("baojia:"+this.sysUserId)
     let sendDate={
         offerOrderId:this.offerOrderId,
         sysUserId:this.sysUserId
        }
        home.Drugquotation(sendDate).then(response=>{
  ...
  methods:{
    ...
    submission(offerOrderId){//提交
       ...
       $(event.target).css("background","#DA2F1E")
        ...
        this.datas.push({'sysUserId':this.sysUserId,'offerOrderId':offerOrderId,'freight':freights,'words':this.words,'medicinalInfo':this.news})
        ...
        home.referring(datas).then(response=>{ 
        var that=this   
          if(response.data.code=="0000"){
           ...
           setTimeout(function(){
               $(".istoast").fadeOut()
               that.$router.push({name:'home',query: {Inquiry3:4}}); 
           },2000);
            $(".istoast").fadeIn()
    ...
    keep(event,offerOrderInfoId,medicinalId,i){//保存
      console.log(i)
      ...
      if(this.money!="" && this.num!=="" && effectperiod!="" || norms2!==""){
       ...
       // this.news=[]
      this.news.push({'offerOrderInfoId':offerOrderInfoId,'num':this.num,'norms':norms2,'invalidTime':effectperiod,'unit':unit,'smallUnit':smallUnit,'price':this.money,'scaler':scaler,})
