  ...
  <div>
    ...
    <div class="home_content">
      <div class="rebate_menu clearfix">
        <div class="home_name home_level" v-for="(item,index) in arrFull" @click="showNum(item.acMeQrcode,$event)">
          <ul>
            ...
            <li class="home_agio"><span>「满{{item.miniNum}}盒赠</span><font class="home_tip">{{item.giftName}}</font><span>*{{item.giftNum}}」</span></li>
            <li class="home_site"><img src="../../assets/public/home_location.png"/><span>{{item.distributionArea}}</span></li>
            <li class="home_money"><span>¥</span><font>{{item.intPrice}}</font><span>.{{item.doublePrice}}</span></li>
          </ul>
        </div>
      </div>
    </div>
        ...
        home.saleData(sendDate).then(res=>{
          if(res.data.code=="0000"){
            ...
            for (var i=0;i<res.data.data.length;i++){
              this.arrFull.push(res.data.data[i]);
            }
//            商家名称
...
<style>
  ...
  .rebate_banner{ width: 100%; margin-bottom: 20px; background: url("../../assets/public/gifts_banner.png") no-repeat center top #A72CEA; height: 200px;}
